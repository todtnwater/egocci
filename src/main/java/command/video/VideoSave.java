...
public class VideoSave implements CommonExecute {
    @Override
    public void execute(HttpServletRequest request) {
        ...
        try {
            ...
            boolean result = dao.saveVideo(dto);

            request.setAttribute("t_msg", result ? "영상이 등록되었습니다." : "등록 실패!");
            request.setAttribute("t_url", "Video?t_gubun=list");

        } catch (Exception e) {
            System.err.println("[VideoSave] 오류: " + e.getMessage());
            e.printStackTrace();
