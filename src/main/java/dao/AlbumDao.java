...
public class AlbumDao {
    ...
    public boolean updateAlbum(AlbumDto dto) {
        try {
            con = DBConnection.getConnection();
            String sql = "UPDATE albums SET album_title = ?, album_number = ?, release_date = ?, album_description = ? " +
                         "WHERE album_id = ?";
            ps = con.prepareStatement(sql);
            ps.setString(1, dto.getAlbum_title());
            ps.setString(2, dto.getAlbum_number());
            ps.setString(3, dto.getRelease_date());
            ps.setString(4, dto.getAlbum_description());
            ps.setInt(5, dto.getAlbum_id());
            return ps.executeUpdate() > 0;
        } catch(Exception e) {
            System.out.println("updateAlbum 오류: " + e.getMessage());
            e.printStackTrace();
            return false;
        } finally {
            DBConnection.closeDB(con, ps, rs);
